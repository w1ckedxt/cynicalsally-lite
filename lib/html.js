      ...
      try {
        ...
        link.onclick = function(e) {
          ...
          fetch('/api/card', {
            ...
            .catch(function() { window.open(burncardUrl, '_blank'); });
        };
        ...
        // Create card for X share async, fallback to image URL
        (function() {
          fetch('/api/card', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              subject: cardSubject, inputType: 'url',
              premium: false, source: 'cli',
            }),
          })
            .then(function(r) { return r.json(); })
            .then(function(data) {
              var shareLink = data.url || burncardUrl;
