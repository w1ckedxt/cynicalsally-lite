...
export async function fetchGitHubFiles(owner, repo, path) {
  ...
  if (!repoRes.ok) {
    ...
    if (repoRes.status === 403) {
      ...
      const tokenHint = GITHUB_TOKEN
        ? `Try again in ~${resetMin} minutes.`
        : `Add your GitHub Personal Access Token to the GITHUB_TOKEN variable in your Render dashboard (Environment tab) to increase the limit from 60 to 5,000 requests/hr.`;
      throw new Error(`GitHub rate limit exceeded. ${tokenHint}`);
    }
  ...
  try {
    ...
    console.error(`GitHub API returned non-JSON (status ${repoRes.status}): ${repoText.slice(0, 200)}`);
    throw new Error("GitHub API returned an unexpected response. Try again.");
  }
  ...
  try {
    ...
    console.error(`GitHub tree API returned non-JSON (status ${treeRes.status}): ${treeText.slice(0, 200)}`);
    throw new Error("GitHub API returned an unexpected response. Try again.");
  }
